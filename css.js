...
sf.color.calc = function (a, b, p){
	return [0, 1, 2].map((i) => b[i] + (a[i] - b[i]) * p);
};

// Color range n between 2 RGB values a and b
sf.color.range = function (a, b, n){
	var cRange = [];
	for (i = 0; i < n; i++){
		cRange[i] = sf.color.calc(a, b, i / (n - 1));
	};
	return cRange;
};
...
if (dm == "default"){
	dm = { // style settings
		initialized: 0, // first time use / no stored value found
		active: true,
		theme: { // what css to load
			preset: "black",
			overlay: true
		},
		color: { // changeable color settings
			// backgrounds colors
			bg: {
				max: [0, 0, 0], // black
				min: [100, 100, 100], // dark gray
				transparent: "rgba(0,0,0,0),"
			},
			// text colors
			text: { // main text color
				max: [200, 200, 200], // light gray
				min: [255, 255, 255], // white
				contrast: "#ffb100",
				url: "###############################"
			},
			// blue theme colors
		...
		// for future custom CSS option
		custom: {
			active: false
		}
	};
	dm.color.bg.range = sf.color.range(dm.color.bg.min, dm.color.bg.max, 5);
	dm.color.text.range = sf.color.range(dm.color.text.min, dm.color.text.max, 3);
	dm.color.text.likebg = dm.color.bg.max;
	dm.color.duoblue.range = sf.color.range(dm.color.duoblue.min, dm.color.duoblue.max, 3);
	dm.color.correct.range = sf.color.range(dm.color.correct.min, dm.color.correct.max, 3);
	dm.color.wrong.range = sf.color.range(dm.color.wrong.min, dm.color.wrong.max, 2);
	dm.color.gold.range = sf.color.range(dm.color.gold.min, dm.color.gold.max, 2);
};
...
pc.bg.secondary		= 	"var(--secondarybg)";
pc.bg.main			= 	"var(--mainbg)";
pc.bg.darker		= 	"var(--darkerbg)";
pc.bg.dark			=	"var(--darkbg)";
pc.bg.darkest		=	"var(--darkestbg)";
pc.bg.transparent	=	dm.color.bg.transparent;

pc.duoblue.main		= 	"var(--duoblue)";
pc.duoblue.dark		= 	"var(--darkblue)";
pc.duoblue.darkest	=	"var(--darkestblue)";

pc.text.main		= 	"var(--maintext)";
pc.text.light		= 	"var(--lightertext)";
pc.text.lightest	= 	"var(--lighttext)";

pc.text.likebg		= 	"var(--textlikebg)";
pc.text.contrast	= 	"var(--contrasttext)";

pc.correct.main		= 	"var(--correct)";
pc.correct.dark		=	"var(--correctdark)";
pc.correct.light	=	"var(--correctlight)";

pc.wrong.main		= 	"var(--wrong)";
pc.wrong.dark		=	"var(--wrongdark)";

pc.gold.light		=	"var(--goldlight)";
pc.gold.dark		=	"var(--golddark)";
...
var styleVars = ':root{' +
	// background
	'--mainbg: rgb(' + 			dm.color.bg.range[0]		+ ');' +
	'--secondarybg: rgb(' + 	dm.color.bg.range[4]		+ ');' +
	'--darkerbg: rgb(' + 		dm.color.bg.range[3]		+ ');' +
	'--darkbg: rgb(' + 			dm.color.bg.range[2]		+ ');' +
	'--darkestbg: rgb(' + 		dm.color.bg.range[1]		+ ');' +
	// text
	'--maintext: rgb(' + 		dm.color.text.range[0]		+ ');' +
	'--lightertext: rgb(' + 	dm.color.text.range[1]		+ ');' +
	'--lighttext: rgb(' + 		dm.color.text.range[2]		+ ');' +
	'--textlikebg: rgb(' + 		dm.color.text.likebg		+ ');' +
	'--contrasttext: rgb(' +	dm.color.text.contrast		+ ');' +
	// blue accent color
	'--duoblue: rgb(' +			dm.color.duoblue.range[0]	+ ');' +
	'--darkblue: rgb(' +		dm.color.duoblue.range[1]	+ ');' +
	'--darkestblue: rgb(' +		dm.color.duoblue.range[2]	+ ');' +
	// correct color
	'--correctlight: rgb(' +	dm.color.correct.range[0]	+ ');' +
	'--correct: rgb(' +			dm.color.correct.range[1]	+ ');' +
	'--correctdark: rgb(' +		dm.color.correct.range[2]	+ ');' +
	// wrong color
	'--wrong: rgb(' +			dm.color.wrong.range[0]	+ ');' +
	'--wrongdark: rgb(' +		dm.color.wrong.range[1]	+ ');' +
	// gold color
	'--goldlight: rgb(' +		dm.color.gold.range[0]		+ ');' +
	'--golddark: rgb(' +		dm.color.gold.range[1]		+ ');' +
'}';
...
darkicons.mobile.inac.tree = imgsvg + "<svg xmlns='http://www.w3.org/2000/svg' width='46' height='46' viewBox='0 0 46 46' version='1.1'><head xmlns=''/><head xmlns=''/><head xmlns=''/><title>learn_inactive</title><g stroke='none' stroke-width='1' fill='none' fill-rule='evenodd'><g fill='rgb(" + dm.color.bg.range[4] + ")' stroke='rgb(" + dm.color.text.range[0] + ")' stroke-width='2.27'><g transform='translate(6 3)'><ellipse cx='15' cy='33.464' rx='5.865' ry='5.758'/><ellipse cx='24.5' cy='17.522' rx='7.365' ry='7.235'/><ellipse cx='11.082' cy='10.907' rx='9.941' ry='9.772'/></g></g></g><script xmlns=''/></svg>";

darkicons.mobile.inac.face = imgsvg + "<svg xmlns='http://www.w3.org/2000/svg' width='46' height='46' viewBox='0 0 46 46' version='1.1' style='&%2310;&%2310;'><head xmlns=''/><head xmlns=''/><head xmlns=''/><title>profile_inactive</title><g stroke='none' stroke-width='1' fill='none' fill-rule='evenodd'><g><path d='M39.564 28.544l-.697-.29V21.5c0-8.487-6.88-15.367-15.367-15.367-8.487 0-15.367 6.88-15.367 15.367v6.753l-.697.29A5.368 5.368 0 0 0 9.5 38.868h28a5.367 5.367 0 0 0 2.064-10.323z' stroke='rgb(" + dm.color.text.range[0] + ")' stroke-width='2.266' fill='rgb(" + dm.color.bg.range[4] + ")'/><g><g><g><path d='M23.5 10.133c-6.278 0-11.367 5.09-11.367 11.367v6c0 6.278 5.09 11.367 11.367 11.367 6.278 0 11.367-5.09 11.367-11.367v-6c0-6.278-5.09-11.367-11.367-11.367z' stroke='rgb(" + dm.color.text.range[0] + ")' stroke-width='2.266' fill='rgb(" + dm.color.bg.range[4] + ")'/><g transform='translate(18.987 18.9)' fill='rgb(" + dm.color.text.range[0] + ")'><path d='M1.513.1a1.5 1.5 0 0 1 1.5 1.5v4a1.5 1.5 0 0 1-3 0v-4a1.5 1.5 0 0 1 1.5-1.5z'/><rect x='6.013' y='.1' width='3' height='7' rx='1.5'/></g></g></g></g><path fill='rgb(" + dm.color.bg.range[4] + ")' d='M27.02 8.78l.703.833v2.819l-3.647 3.78-5.12 2.69-5.694.962-4.137.198.688-1.968 2.041-3.772 2.56-3.3 5.567-2.754h4.799z'/><path d='M8 20.468h4.12c5.302-.036 13.74-.79 16.49-10.492.14-.57.276-1.228.406-1.976' stroke='rgb(" + dm.color.text.range[0] + ")' stroke-width='2.266'/><path d='M31.156 11.5c-.875-.708-2.52-1.396-4.937-2.063l1.093-1.687c2.042.625 3.292 1.188 3.75 1.688.688.75 1.407 3.124.094 2.062zM11.25 18.188c-.125 2.125-.188 3.562-.188 4.312 0 .75-.604.75-1.812 0 0-2.083.125-3.417.375-4 .25-.583.792-.688 1.625-.313z' fill='rgb(" + dm.color.bg.range[4] + ")'/></g></g><script xmlns=''/></svg>";

darkicons.mobile.inac.treasure = imgsvg + "<svg xmlns='http://www.w3.org/2000/svg' width='46' height='46' viewBox='0 0 46 46' version='1.1'><head xmlns=''/><head xmlns=''/><head xmlns=''/><title>shop_inactive</title><g stroke='none' stroke-width='1' fill='none' fill-rule='evenodd'><g><path d='M35.486 8.163V9.33H10.514V8.163c0-.568-.46-1.028-1.029-1.028H5.163c-.568 0-1.028.46-1.028 1.028v29.674c0 .568.46 1.028 1.028 1.028h35.674c.568 0 1.028-.46 1.028-1.028V8.163c0-.568-.46-1.028-1.028-1.028h-4.322c-.568 0-1.029.46-1.029 1.028z' stroke='rgb(" + dm.color.text.range[0] + ")' stroke-width='2.27' fill='rgb(" + dm.color.bg.range[4] + ")'/><path d='M2 23h43' stroke='rgb(" + dm.color.text.range[0] + ")' stroke-width='2.27' stroke-linecap='round' stroke-linejoin='round'/><path d='M17.163 15.135c-.568 0-1.028.46-1.028 1.028v14.674c0 .568.46 1.028 1.028 1.028h11.674c.568 0 1.028-.46 1.028-1.028V16.163c0-.568-.46-1.028-1.028-1.028H17.163z' stroke='rgb(" + dm.color.text.range[0] + ")' stroke-width='2.27' fill='rgb(" + dm.color.bg.range[4] + ")'/><g transform='translate(20 18)' fill='rgb(" + dm.color.text.range[0] + ")'><ellipse cx='3' cy='3.676' rx='2.692' ry='2.746'/><path d='M4.346 5.324l.918 2.996A1.082 1.082 0 0 1 4.23 9.718H1.77A1.082 1.082 0 0 1 .736 8.32l.918-2.996a1.408 1.408 0 0 1 2.692 0z'/></g></g></g><script xmlns=''/></svg>";


darkicons.menu = {};
darkicons.menu.active = "<svg xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink' xmlns:a='http://ns.adobe.com/AdobeSVGViewerExtensions/3.0/' version='1.2' baseProfile='tiny' x='0px' y='0px' width='177px' height='202px' viewBox='-0.8 -0.5 177 202' xml:space='preserve'><head xmlns=''/><head xmlns=''/><head xmlns=''/><defs></defs><path fill='none' stroke='rgb(" + dm.color.text.range[0] + ")' stroke-width='30' stroke-linecap='round' d='M33.7,64.3C22.1,77.2,15,94.3,15,113  c0,40.1,32.5,72.7,72.7,72.7c40.1,0,72.7-32.5,72.7-72.7c0-18.7-7.1-35.8-18.7-48.7'/><line fill='none' stroke='rgb(" + dm.color.text.range[0] + ")' stroke-width='30' stroke-linecap='round' x1='87.8' y1='15' x2='87.8' y2='113'/><script xmlns=''/></svg>";
...
'#TrueDarkSettings' +
'{position: fixed;top: -5px;right: -5px;z-index: 9001;color:' + pc.text.lightest + ';}' +
...
'#TDpop' +
'{width:360px;height:auto;min-height:330px;position:absolute;background:' + pc.bg.main + ';top:10px;right:10px;opacity:1;border-radius:20px;border-style: solid;border-color: gray;z-index:300;}' +
...
'.dv' +
'{display:inline;margin-left:10px;margin-top: 1px;float:left;width:30px;height:20px;text-align:right;border:none;background:' + pc.bg.main + ';color:' + pc.text.lightest + '}' +

'.geoLoc' +
'{width: 75px;display: inline;text-align: center;background:' + pc.bg.dark + ';color:' + pc.text.lightest + '}' +
...
'.centerInput' +
'{width: 50%;margin-left: 25%;text-align: center;background:' + pc.bg.dark + ';color:' + pc.text.lightest + '}' +
...
var applyStyles = styleVars;
if (dm){
	if (dm.active == true){
		applyStyles += darkCSS;
	};
	if (dm.custom.active != false){
...
var attempt = 0;
(function addCSSonload(){
	// try ensures the theme loads
	try {
		// add CSSS
		document.head.appendChild(sheet);
		// set CSS selector
		document.documentElement.setAttribute("mode", dm.theme.preset);
		console.log("True Dark loaded after " + attempt++ + " failed attempts.");
	} catch(err) {
